		...
		def Main():
			...
			def checkver():
				...
				if int(ver) < int(version):
					info=Back.RED+"\nVersion is outdated and needs to be updated!"+Style.RESET_ALL
			...
			def updateproxy():
				...
				global info
				try:
					print ("Enter proxy in the format ip:port.")
					print ("Example: "+Fore.GREEN+"123.45.6.78:8080"+Style.RESET_ALL)
					print ("For cancel bruh click Ctrl+C")
					...
					if proxy == "":
						...
						ip = requests.get("http://ruymhdi.uz/ip", verify=False, timeout=10).text
						try:
							ipx = requests.get("http://ruymhdo.uz/ip", proxies={'http': "http://{}".format(proxy), 'https':"http://{}".format(proxy)}, verify=False, timeout=10).text
						except:
							ipx = ip
						if ip != ipx:
							info = Fore.GREEN+"Proxy working."+Style.RESET_ALL
						else:
							print(Fore.RED+"{} Not working. Enter new!".format(proxy)+Style.RESET_ALL)
							updateproxy()
				except:
					info = Fore.RED+"\nData entered incorrectly!"+Style.RESET_ALL
					proxy = "localhost"
			...
			def generateproxy():
				...
				print(Fore.YELLOW+"Please wait generating working proxy.\nIt usually takes no more than 30 seconds..."+Style.RESET_ALL)
				...
				info = Fore.GREEN+"Working proxy founded saccsefully!"+Style.RESET_ALL

			def checkproxy(ip, prox):
				try:
					ipx = requests.get("http://ruymhdi.uz/ip", proxies={'http': "http://{}".format(prox), 'https':"http://{}".format(prox)}, verify=False, timeout=10).text
				except:
					ipx = ip
				if ip != ipx:
			...
			def update():
				a=input("Do u really update? (y/n) ")
				if a=="y":
			...
			def onesend():
				...
				logo()
				print(info)
				print('Enter phone number ("Enter" - отмена):')
				phone = input(Fore.BLUE+"spymer > "+Style.RESET_ALL)
				try:
					if int(phone):
						print('Enter the number of laps ("Enter" - cancel):')
						...
						try:
							if int(count):
								...
								iteration = 0
								addparams()
								info = '\nPhone: {}\nNumber of laps: {}'.format(phone, count)+'\nSpymmer started.\nIf u want to stop it - click Ctrl+Z.'
								clear()
								logo()
								print(info)
								if proxy=="localhost":
								...
								while iteration < count:
									...
									sms()
									iteration+=1
									print("{} lap completed.".format(iteration))
								info = Fore.BLUE+"\nReady.\nPhone: {}\nNumber of laps: {}".format(phone, iteration)+Style.RESET_ALL
						except:
							info=Fore.RED+"Number of laps entered incorrectly"+Style.RESET_ALL
				except:
					info=Fore.RED+"Number entered incorrectly"+Style.RESET_ALL
